...
class NavegaEntrena(Brain):  
   ...
   def calculaVelocidadAvance(self,vgiro):
      return 0.2+0.8*(1-abs(vgiro))

   def setup(self):
      self.rutasUsadas = []
      #self.flag = True
      self.maxTicksPermitidos = 250
      ...
      self.g = 50 #Número máximo de generaciones en algoritmo evolutivo.
      ...
      self.testPath = [((0.37, 1.70, -1.57),(3, 1.70, -1.57))
                       ,((1.70, 3, math.pi),(1.70, 0.6, math.pi))
                       ,((1.70, 0.6, 0),(4.52, 6, 0))
                       ,((1,4.81,math.pi),(4.9,1.8,0))
                       ,((0.37, 1.70, -1.57),(5.5,4.7,0))]
...
#      self.ga = self.ga +1 
   ...
   def controlIteraciones(self,suceso): 
      ...
      if suceso == 'choque' : #Se ha terminado de probar un individuo
         ...
         self.fitness_vector[self.ni] = 0.0
         self.ni = self.ni+1
         if self.ni == self.N: #Se ha caracterizado a toda la población y hay que ir a siguiente generación
            self.ni = -1
         else:
      ...
      if self.ni >= 0: #Si se está calculando fitness
         ...
         if self.robot.stall == 1:
            ...
            #En lo que respecta al entrenamiento, debe gestionarlo
               ...
               teta = t*2*math.pi/360.0-(360.0/(2.0*math.pi))*(math.atan2((self.testPath[self.npath][1][1]-y),(self.testPath[self.npath][1][0]-x))%(2*math.pi))
