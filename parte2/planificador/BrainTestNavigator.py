...
################################
  ...
  def step(self):
    ...
    if (abs(self.pathList[0][0]-self.nav.getCurrentRow())<=1 and
        abs(self.pathList[0][1]-self.nav.getCurrentCol())<=1):
    ...
    son = [self.robot.sonar[0][i].distance() for i in range(0,8)]
    translation, rotate = self.nav.determineMove(self.pathList[0], son)
